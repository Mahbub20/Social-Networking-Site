...
function get_posts(){
	...
	$get_posts = "select *, COUNT(article_likes.id) as likes from posts LEFT JOIN article_likes ON article_likes.article = posts.post_id GROUP BY posts.post_id ORDER by 1 DESC LIMIT $start_from, $per_page";
		...
		if($content=="No" && strlen($upload_image) >= 1){
			...
			<div class='row'>
				...
				<div id='posts' class='col-sm-6'>
                    ...
                    <a href='home.php?type=article&post_id=$post_id' style='float:left;'><button class='btn btn-info'>Like</button></a><br><br>
	...
	if(isset($_GET["type"], $_GET["post_id"])){
		...
		if($type=="article"){
			...
			$query = "INSERT INTO article_likes (user, article) SELECT {$_SESSION['u_id']}, {$id} FROM posts WHERE EXISTS(SELECT post_id FROM posts WHERE post_id = {$id})
			AND NOT EXISTS(SELECT id FROM article_likes WHERE user = {$_SESSION['u_id']} AND id = {$id}) LIMIT 1";
 	...
 	if(isset($_GET['post_id'])){
        ...
        echo "
         ...
         <div class='panel panel-info'>
          ...
          <button class='btn btn-info putll-right' name='reply'>Comment</button>
          </form>
          </div>
         </div>
         </div>
